...
{
    public class StandardShapes : IDisposable
    {
        ...
        public StandardShapes(Game game)
        {
            _game = game;
            ...
            TranslateProjection(0, viewport.Width, viewport.Height, 0);
        }
        ...
        public void TranslateProjection(float left, float right, float bottom, float top)
        {
            _basicEffect.Projection = Matrix.CreateOrthographicOffCenter(left, right, bottom, top, 0f, 1f);
        }

        public void TranslateView(float x, float y)
        {
            _basicEffect.View = Matrix.CreateTranslation(new Vector3(x, y, 0));
        }
        ...
        public void DrawLine(Vector2 a, Vector2 b, float thickness, Color color)
        {
            ...
            var edge1 = b - a; // vector pointing from a to b
            edge1.Normalize();
            edge1 *= halfThickness;

            var edge2 = -edge1; // vector pointing from b to a
            var n1 = new Vector2(-edge1.Y, edge1.X);
            var n2 = -n1;
            var q1 = a + n2 + edge2;
            var q2 = b + n2 + edge1;
            var q3 = b + n1 + edge1;
