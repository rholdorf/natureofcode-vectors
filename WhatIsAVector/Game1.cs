...
namespace WhatIsAVector
{
    public class Game1 : Game
    {
        ...
        /// <summary>
        /// Console games generally set of a field of view of about 60 degrees,
        /// while PC games often set the field of view higher, in the 80-100
        /// degree range. The difference is generally due to the size of the
        /// screen viewed and the distance from it. The higher the field of
        /// view, the more of the scene that will be rendered on screen.
        /// </summary>
        private readonly float _fieldOfViewDegrees = 60f;

        /// <summary>
        /// Camera Lens.
        /// The Projection Matrix is used to convert 3D view space
        /// to 2D. In a nutshell, this is your actual camera lens and is created
        /// by specifying calling CreatePerspectiveFieldOfView() or
        /// CreateOrthographicFieldOfView().
        /// With Orthographic projection, the size of things remain the same
        /// regardless to their depth within the scene.
        /// For Perspective rendering it simulates the way an eye works, by
        /// rendering things smaller as they get further away.
        /// As a general rule, for a 2D game you use Orthographic, while in 3D
        /// you use Perspective projection.
        /// When creating a Perspective view we specify the field of view
        /// (think of this as the degrees of visibility from the center of your
        /// eye view), the aspect ratio (the proportions between width and
        /// height of the display), near and far plane (minimum and maximum
        /// depth to render with camera… basically the range of the camera).
        /// These values all go together to calculate something called the view
        /// frustum, which can be thought of as a pyramid in 3D space
        /// representing what is currently available.
        /// </summary>
        private Matrix _projectionMatrix;

        /// <summary>
        /// Camera Location.
        /// The View Matrix is used to transform coordinates
        /// from World to View space. A much easier way to envision the View
        /// matrix is it represents the position and orientation of the camera.
        /// It is created by passing in the camera location, where the camera is
        /// pointing and by specifying which axis represents Up in the universe.
        /// XNA uses a Y-up orientation, which is important to be aware of when
        /// creating 3D models.
        /// Blender by default treats Z as the up/down axis, while 3D Studio MAX
        /// uses the Y-axis as Up.
        /// </summary>
        private Matrix _viewMatrix;

        /// <summary>
        /// Object Position / Orientation in 3D scene.
        /// The World matrix is used to position your entity within the scene.
        /// Essentially this is your position in the 3D world.
        /// In addition to positional information, the World matrix can also
        ...
        {
            var attractedTriangles = new List<AttractedTriangle>();
            for (var i = 0; i < howMany; i++)
            {
                var attractedTriangle = new AttractedTriangle(
                    game: this,
                    ...
                    screenHeight: HEIGHT,
                    texture: _arrow);
                attractedTriangle.Velocity = new Vector2().Randomize(_random);
                Components.Add(attractedTriangle);
                attractedTriangles.Add(attractedTriangle);
            }
            ...
            Components.Add(new Attractor(
                ...
                screenWidth: WIDTH,
                screenHeight: HEIGHT,
                attracteds: attractedTriangles.ToArray()));
        }

        private void AddAttractor(int howMany)
        {
            var attractedList = new List<Attracted>();
            ...
            Components.Add(new Attractor(
                ...
                screenWidth: WIDTH,
                screenHeight: HEIGHT,
                attracteds: attractedList.ToArray()));
        }
        ...
        private void AddBouncingBalls(int howMany)
        {
            for (var i = 0; i < howMany; i++)
            {
                Components.Add(new BouncingBall(
                    game: this,
                    position: new Vector2((float)WIDTH / howMany * i, HEIGHT / 1.25f - (_random.Next(0, howMany * 2))),
                    color: Color.White,
